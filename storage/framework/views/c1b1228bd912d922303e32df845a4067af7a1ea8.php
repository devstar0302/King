            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <table class="table table-striped" border="1" id="malfunctions-table">
                        ...
                        <tr>
                            <th data-index='0' data-sort="string" class="sortStyle"><?php echo e(__('Form')); ?> #</th>
                            <th data-index='1' data-sort="string" class="sortStyle"><?php echo e(__('Date')); ?></th>
                        ...
                        <tbody>
                            <?php $__currentLoopData = $malfunctions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $malfunction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                ?>
                                <tr data-type='malfunction' data-id='<?php echo e($malfunction->id); ?>' class="<?php echo e($admin_changed); ?>">
                                    <td><a href="<?php echo e($link_address); ?>"><?php echo e($malfunction->nameCode); ?></a></td>
                                    ...
                                    <td class='status'>
                                        <?php if(isset($malfunction->data['status']) && isset($malfunction->data['status']['stage']) &&
                                            ...
                                            <label class="toggle">
                                                <input type="checkbox" name="toggle-status" <?php if(isset($malfunction->data['status']) && isset($malfunction->data['status']['stage']) && $malfunction->data['status']['stage'] == 'publish'): ?> checked='checked' <?php endif; ?> }} <?php if($user_role != 'admin'): ?> disabled <?php endif; ?>>
                                                <i data-swchon-text="<?php echo e(__('ON')); ?>" data-swchoff-text="<?php echo e(__('OFF')); ?>"></i>
                                            </label>
                                        <?php endif; ?>
                                        <?php if(!isset($malfunction->data['status']) || !isset($malfunction->data['status']['stage']) || $malfunction->data['status']['stage'] == 'draft'): ?>
                                            <img src="<?php echo e(url('/')); ?>/img/draft.png" width=40 style='margin-left:5px;'>
                                        <?php endif; ?>
                                        <?php if($user_role == 'admin' && isset($malfunction->data['employee_id']) && isset($malfunction->data['status']) && isset($malfunction->data['status']['admin_changed_date'])): ?>
                                            <?php $employee_id = $malfunction->data['employee_id']; ?>
                                            <?php if(isset($malfunction->data['status']['users']) && isset($malfunction->data['status']['users'][$employee_id]) && strtotime($malfunction->data['status']['users'][$employee_id]['last_visit_date']) > strtotime($malfunction->data['status']['admin_changed_date'])): ?>
                                                <i class='fa fa-thumbs-up'></i>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                        <?php if(isset($malfunction->data['status']) && isset($malfunction->data['status']['stage'])): ?>
                                            <?php if(isset($malfunction->data['status']['last_comment_date']) && (!isset($malfunction->data['status']['users']) || !isset($malfunction->data['status']['users'][$user_id]) || strtotime($malfunction->data['status']['users'][$user_id]['last_visit_date']) < strtotime($malfunction->data['status']['last_comment_date']))): ?>
                            ...
                            <?php $__currentLoopData = $guidances; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $guidance): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                ?>
                                <tr data-type='guidance' data-id='<?php echo e($guidance->id); ?>' class="<?php echo e($admin_changed); ?>">
                                    <td><a href="<?php echo e($link_address); ?>"><?php echo e($guidance->nameCode); ?></a></td>
                                    <td data-sort-value='<?php echo e($date_sort_value); ?>'><?php echo e($date); ?></td>
